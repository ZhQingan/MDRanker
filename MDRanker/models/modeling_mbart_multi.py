...
class MBartDecoderMulti(MBartPreTrainedModel):
    """
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([MBartDecoderLayer(config) for _ in range(config.decoder_layers+2)])
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self.layernorm_embedding = nn.LayerNorm(config.d_model)
        # self.layer_norms = nn.ModuleList([nn.LayerNorm(config.d_model) for _ in range(3)])
        self.layer_norm = nn.LayerNorm(config.d_model)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if idx < len(self.layers) - 3 :
                ...
                if idx == len(self.layers) - 3:
                    hidden_states_multi += (self.layer_norm(layer_outputs[0]),)
                elif idx == len(self.layers) - 2:
                    hidden_states_multi += (self.layer_norm1(layer_outputs[0]),)
                elif idx == len(self.layers) - 1:
                    hidden_states_multi += (self.layer_norm2(layer_outputs[0]),)
                # hidden_states_multi += (self.layer_norms[idx-len(self.layers)+3](layer_outputs[0]),)
        ...
        # coef = self.head_selector(hidden_states)
        # coef = nn.functional.softmax(coef, dim=-1)
        ...
        if not return_dict:
            # return tuple(
            #     v
            #     for v in [hidden_states_multi, coef, next_cache, all_hidden_states, all_self_attns, all_cross_attentions]
            #     # for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_cross_attentions]
            #     if v is not None
            # )
            tup = tuple(
                v
                for v in [hidden_states_multi, next_cache, all_hidden_states, all_self_attns, all_cross_attentions]
                # for v in [hidden_states_multi, coef, next_cache, all_hidden_states, all_self_attns, all_cross_attentions]
                # for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_cross_attentions]
                if v is not None
            )
        ...
        return BaseModelOutputWithPastAndCrossAttentions(
            # last_hidden_state=hidden_states,
            last_hidden_state=hidden_states_multi,
            past_key_values=next_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attns,
            cross_attentions=all_cross_attentions,
...
class CustomMBartModel(MBartPreTrainedModel):
    def __init__(self, config: MBartConfig):
        ...
        self.encoder = MBartEncoder(config, self.shared)
        self.decoder = MBartDecoderMulti(config, self.shared)
...
class MBartScorer(MBartPreTrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]
    _keys_to_ignore_on_load_missing += [
        r"decoder\.layers\.%d\.%s_attn\.%s_proj\.%s" % (i, t, p, q) 
        for i in range(12,14) for t in ['self','encoder'] for p in ['k','v','q','out'] for q in ['weight', 'bias']
    ]
    _keys_to_ignore_on_load_missing += [
        r"decoder\.layers\.%d\.%s_layer_norm\.%s" % (i, t, q) 
        for i in range(12,14) for t in ['self_attn','encoder_attn',"final"] for q in ['weight', 'bias']
    ]
    _keys_to_ignore_on_load_missing += [
        r"decoder\.layers\.%d\.fc%d\.%s" % (i, t, q) 
        for i in range(12,14) for t in range(1,3) for q in ['weight', 'bias']
    ]
    _keys_to_ignore_on_load_missing +=  ['model.decoder.layer_norm1.bias', 'model.decoder.layer_norm1.weight', 'model.decoder.layer_norm2.bias', 'model.decoder.layer_norm2.weight']
    ...
    def __init__(self, config: MBartConfig):
        ...
        self.C = nn.Parameter(torch.zeros(3))
        self.post_init()
    ...
    def forward(
        ...
        for dim, hs in enumerate(outputs[0]):
            lm_logits_mul += (F.softmax(self.lm_head(hs) + self.final_logits_bias, dim=-1),)
        
        if self.logits_merge == "trainable":
            # multi_trainable
            lm_logits = lm_logits_mul[0]*min(1, self.A) + lm_logits_mul[1]*min(1, self.B) + lm_logits_mul[2]*max(0,(1-self.A-self.B))
        
        elif self.logits_merge == "softmax":
            coef = F.softmax(self.C, dim=-1)
            lm_logits = lm_logits_mul[0]*coef[0] + lm_logits_mul[1]*coef[1] + lm_logits_mul[2]*coef[2]

        lm_logits_mul += (lm_logits,)
        ...
        if not return_dict:
            output = (lm_logits_mul,)
            return ((masked_lm_loss,) + output) if masked_lm_loss is not None else output

        return Seq2SeqLMOutput(
            loss=masked_lm_loss,
            logits=lm_logits_mul,
            past_key_values=outputs.past_key_values,
            decoder_hidden_states=outputs.decoder_hidden_states,
            decoder_attentions=outputs.decoder_attentions,
            cross_attentions=outputs.cross_attentions,
