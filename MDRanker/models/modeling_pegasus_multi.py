...
class PegasusDecoderMulti(PegasusPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a :class:`PegasusDecoderLayer`
    ...
    def __init__(self, config: PegasusConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([PegasusDecoderLayer(config) for _ in range(config.decoder_layers+2)])
        self.layer_norm = nn.LayerNorm(config.d_model)
...
class CustomPegasusModel(PegasusPreTrainedModel):
    def __init__(self, config: PegasusConfig):
        ...
        self.encoder = PegasusEncoder(config, self.shared)
        self.decoder = PegasusDecoderMulti(config, self.shared)
...
class PegasusScorer(PegasusPreTrainedModel):
    ...
    _keys_to_ignore_on_load_missing += [
        ...
        for i in range(16,18) for t in ['self','encoder'] for p in ['k','v','q','out'] for q in ['weight', 'bias']
    ]
    _keys_to_ignore_on_load_missing += [
        r"decoder\.layers\.%d\.%s_layer_norm\.%s" % (i, t, q) 
        for i in range(16,18) for t in ['self_attn','encoder_attn',"final"] for q in ['weight', 'bias']
    ]
    _keys_to_ignore_on_load_missing += [
        r"decoder\.layers\.%d\.fc%d\.%s" % (i, t, q) 
        for i in range(16,18) for t in range(1,3) for q in ['weight', 'bias']
    ]
